...
class AICodeReviewer(BaseReviewer):
    """AI-powered code reviewer supporting multiple AI backends."""

    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.backend = config.get("ai_backend", "ollama")
        self.config = config
    ...
    def _get_ai_review(self, prompt: str) -> Dict[str, Any]:
        """
        Sends a code review prompt to the configured AI backend and returns the parsed reviews.
        ...
        """
        if self.backend == "azure_openai":
            return self._get_openai_review(prompt)
        elif self.backend == "ollama":
            return self._get_ollama_review(prompt)
        else:
            raise ValueError(f"Unsupported AI backend: {self.backend}")
    ...
    def _get_openai_review(self, prompt: str) -> Dict[str, Any]:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a Senior Software Engineer and have been assigned to review this PR. Provide specific, actionable feedback that helps improve code quality."
            },
            {
                "role": "user",
                "content": prompt
    ...
    def _get_ollama_review(self, prompt: str) -> Dict[str, Any]:
        """
        Sends a code review prompt to Ollama and returns the parsed reviews.
        ...
        try:
            response = requests.post(
                ...
                json={"model": "code-review", "prompt": prompt},
                timeout=30
            )
    ...
    def review_file(self, file_path: str, hunks: List[Dict[str, Any]], pr_details: PRDetails) -> List[Dict[str, Any]]:
        ...
        for hunk_data in hunks:
            ...
            reviews = self._get_ai_review(prompt)
